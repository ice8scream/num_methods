...
function showMatx(matx, name = 'ENY'){
    console.log(`\n/====${name}====\\`);
...
function Holecki_SquareRoots() {
    function createLeftSquereMatx(matx) {
        //console.log(matx);
        let L = matx.map((item) => item.map(() => 0));
        //showMatx(L);
        matx.forEach((item, i) =>
            item.forEach((item, j) => {
                if (i < j) {
                ...
                L[i][j] = i === j ? Math.sqrt(cell): cell / L[j][j];
                // if (i === j) {
                //     L[i][j] = Math.sqrt(cell);
                // } else {
                //     L[i][j] = cell / L[j][j];
                // }
            })
    ...
    let L = createLeftSquereMatx(inputVar.A);
    let U = myMath.matrixTranspone(L);
    //let ans = myMath.matxMultiply(L,U);
    //showMatx(myMath.reverseMatx(L));
    let y = myMath.matxAndVectorMultiply(myMath.reverseMatx(L), inputVar.b);
    //console.log(y);
    let x = myMath.matxAndVectorMultiply(myMath.reverseMatx(U), y);
    console.log(x);
}
